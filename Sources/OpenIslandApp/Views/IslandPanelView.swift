...
struct IslandPanelView: View {
    ...
    private func openedContent(width: CGFloat) -> some View {
        // No companion here. It stood directly above `sessionPanelHeader`, whose
        // overview chips already answer "does anything need me" numerically and
        // whose `shippedTodayBadge` already prints the day's tally — two summary
        // strips 8pt apart saying the same two things. That is the same
        // duplication that sank the five-creature band, one level up. The
        // companion lives in the pill and on its own surface; the panel is a
        // tool and goes back to being one.
        VStack(spacing: 8) {
